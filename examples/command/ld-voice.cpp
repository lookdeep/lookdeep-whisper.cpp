...
int always_prompt_transcription(struct whisper_context * ctx, audio_async & audio, const whisper_params & params) {
    ...
    zmq::socket_t pub_socket(zmq_ctx, zmq::socket_type::pub);
    pub_socket.connect("tcp://127.0.0.1:5560");
    fprintf(stdout, "zmq connected\n");
    ...
    while (is_running) {
...

        {
            ...
            if (::vad_simple(pcmf32_cur, WHISPER_SAMPLE_RATE, 1000, params.vad_thold, params.freq_thold, params.print_energy)) {
                ...
                if (words.size() < k_prompt_length) {
                    fprintf(stdout, "command too short. num words: %d\n", words.size());
                }
